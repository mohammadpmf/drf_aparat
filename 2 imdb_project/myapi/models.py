...
class Movie(models.Model):
    ...
    year = models.PositiveIntegerField(
        validators=[MinValueValidator(1920), MaxValueValidator(2040)]
    )
    runtime = models.PositiveIntegerField()  # in minutes
    status = models.CharField(
        max_length=8, choices=STATUS_MOVIES, default=STATUS_RELEASED
    )
    director = models.ForeignKey(
        to=Artist, on_delete=models.PROTECT, related_name="movies_directed"
    )
...
class Serial(models.Model):
    ...
    end_year = models.PositiveIntegerField(
        validators=[MinValueValidator(1920)], blank=True, null=True
    )
    average_runtime = models.PositiveIntegerField()  # in minutes
    director = models.ForeignKey(
        to=Artist, on_delete=models.PROTECT, related_name="series_directed"
    )
    country = models.CharField(max_length=64, blank=True)
    plot = models.TextField(blank=True)
...
class Music(models.Model):
    title = models.CharField(max_length=256)
    year = models.PositiveIntegerField(
        validators=[MinValueValidator(1800), MaxValueValidator(2040)]
    )
    duration = models.PositiveIntegerField()  # in seconds
    main_singer = models.ForeignKey(
        to=Artist, on_delete=models.PROTECT, related_name="main_musics"
    )
...
class Image(models.Model):
    ...
    movie = models.ForeignKey(
        to=Movie, on_delete=models.CASCADE, related_name="images", blank=True, null=True
    )
